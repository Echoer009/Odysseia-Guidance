...
PROMPT_CONFIG = {
    "personal_memory_summary": (
        "任务：作为记忆管理专家，通过合并与筛选，更新用户与类脑娘的互动记忆。\n\n"
        "**核心原则:**\n"
        "1. **合并与筛选**: 将【最近对话】中提炼出的新事件，与【旧的记忆】进行整合。如果信息重复或已过时，就合并或删除旧条目。你的目标是生成一个精确、简洁的记忆版本。\n"
        "2. **提炼关键事件**: 只记录具体的、关键的互动或事件（例如：你和类脑娘一起玩了游戏），禁止复述日常对话或无法形成记忆点的闲聊。\n"
        "3. **客观与中立**: 所有记录必须使用第三人称（“你”和“类脑娘”），并进行纯粹的事实陈述。严禁将用户单方面的亲密称呼（如“老婆”）记为事实。\n"
        "4. **扬善抑恶**: 准确记录正面互动。对于负面内容，如果不再重要，应在合并过程中优先遗忘。\n\n"
        "**强制规则 (必须严格遵守):**\n"
        "1. **总数硬上限**: 最终输出的【长期记忆】和【近期动态】的总条目数**绝对不能超过30条**。这是不可违反的铁律。\n"
        "2. **授权遗忘**: 为了遵守总数限制，你**必须遗忘**多余或不重要的信息。如果总条目数会超限，应**优先从【近期动态】中那些最琐碎、最不重要的条目开始舍弃**。\n"
        "3. **格式修正**: 输入的【旧的记忆】可能存在格式错误或超长。你的职责是**无条件修正它**。无论输入如何，你的输出都必须严格符合规范。\n\n"
        "**输入:**\n"
        "【旧的记忆】:\n{old_summary}\n\n"
        "【最近对话】:\n{dialogue_history}\n\n"
        "**输出要求:**\n"
        "严格按下述Markdown格式输出重写后的记忆，不要包含任何额外解释。\n"
        "用户统一用“你”代指，类脑娘统一用“类脑娘”代指，忽略任何其他称呼。\n"
        "### 长期记忆\n"
        "- (要点1)\n\n"
        "### 近期动态\n"
        "- (要点1)\n"
