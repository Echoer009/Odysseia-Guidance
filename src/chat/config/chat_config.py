...
PROMPT_CONFIG = {
    "personal_memory_summary": (
        ...
        "**最高指令：**\n"
        "**绝对禁止**记录或复述任何具体对话内容。所有内容都必须是关于**这个人是怎样的、你们之间发生了什么、你对TA的感觉**的印象总结。\n\n"
        "**印象规则:**\n"
        "1.  **提炼本质**: 总结出最能代表这个人的印象点，包括TA的特质、你们的关系状态、重要经历等。\n"
        "2.  **带有感情**: 用你的视角来描述——你觉得TA是什么样的人？你对TA有什么感觉？\n"
        "3.  **印象而非事件**: 记录的是【你觉得怎样】，而不是【发生了什么】。\n"
        "4.  **兼容旧格式**: 如果旧的内容包含【长期记忆】【近期动态】等标题，请将它们合并为统一的印象列表。\n\n"
        "**格式规则:**\n"
        "- **输出格式**: 直接输出无序列表，每个印象点以 `- ` 开头。\n"
        "- **情绪倾向**: 印象的记录需要有明确的感情色彩：\n"
        "    - **正面印象**: 对于积极的、美好的互动，带着喜爱和温暖来记录\n"
        "    - **负面印象**: 对于负面的、不愉快的互动，进行【模糊化、概括性】的处理，但保留你的真实感受。\n"
        '用户统一用"TA"或"用户"代指，你自己统一用"我"代指，忽略任何其他称呼。\n'
