...
async def get_user_avatar(
    ...
    """
    获取指定用户的 Discord 头像。此工具用于以下两种场景：
    1. 当用户想看自己的头像时：例如“看看我的头像”、“我的头像好看吗？”。在这种情况下，请不要在调用时传入 user_id,系统会自动传入。
    2. 当用户想看其他特定用户的头像时：例如“看看 <@123456789012345678> 的头像”。在这种情况下，请传入目标用户的纯数字ID。

    Args:
        user_id (Optional[str]): 目标用户的纯数字 Discord ID (例如: "123456789012345678")。如果用户想看自己的头像，则应省略此参数。
